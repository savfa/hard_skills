...
## Self-Check
1. Что такое FSD (Feature-Sliced Design) и какие проблемы он решает по сравнению с "обычной" структурой папок?
2. В чем принципиальная разница между `tree shaking` и `code splitting`?
3. Почему важно использовать MSW в тестах вместо моканья API на уровне функций?
4. Какие преимущества дает монорепозиторий? Какие у него есть минусы?
5. Как настроить автоматическую проверку линтера при попытке сделать коммит?
